...
class RobotController():
    ...
    def set_goal_impedance(self, action):
        ...
        if self.use_delta_impedance:
            ...
            self.goal_kp = np.clip(self.impedance_kp[self.action_mask] + action[self.kp_index[0]:self.kp_index[1]], self.kp_min, self.kp_max)
            self.goal_damping = np.clip(self.damping[self.action_mask] + action[self.damping_index[0]:self.damping_index[1]], self.damping_min, self.damping_max)
        else:
...
class JointImpedanceController(RobotController):
    ...
    def __init__(self,
        ...
        # for backcompatibility interpret a single # as the same value for all joints
    ...
    def interpolate_joint(self, starting_joint, last_goal_joint, goal_joint, current_vel):
        ...
        if self.interpolation_type == "cubic":
            ...
            self.spline_joint = CubicSpline(time, position, bc_type=((1, current_vel), (1, (0,0,0,0,0,0,0))), axis=0)
        elif self.interpolation_type == 'linear':
            delta_x_per_step = (goal_joint - last_goal_joint)/self.interpolation_steps
            self.linear_joint = np.array([(last_goal_joint + i*delta_x_per_step) for i in range(1,int(self.interpolation_steps)+1)])
        else:
            logger.error("[Controller] Wrong interpolation_type!")
            exit(-1)
    ...
    def action_to_torques(self, action, policy_step):
        ...
        if policy_step == True:
            ...
            if self.interpolate:
                ...
                self.interpolate_joint(self.current_joint_position, self.last_goal_joint, self.goal_joint_position, self.current_joint_velocity)
        ...
        if self.interpolate:
            ...
            if self.interpolation_type == 'linear':
                ...
                raise Exception("[Controller] Wrong interpolation_type!")
...
class PositionOrientationController(RobotController):
    ...
    def interpolate_position(self, starting_position, last_goal_position, goal_position, current_vel):
        ...
        if self.interpolation_type == "cubic":
            ...
            self.linear_pos = np.array([(last_goal_position + i*delta_x_per_step) for i in range(1,int(self.interpolation_steps)+1)])
        else:
            raise Exception("[Controller] Wrong interpolation_type!")
    ...
    def interpolate_orientation(self, starting_orientation, last_goal_orientation, goal_orientation, current_vel):
        ...
        if self.interpolation_type == "cubic":
            ...
            orientation_error = self.calculate_orientation_error(desired=goal_orientation, current= last_goal_orientation)
    ...
    def action_to_torques(self, action, policy_step):
        ...
        if policy_step == True:
            ...
            if self.interpolate:
                ...
                self.interpolate_position(self.current_position, self.last_goal_position, self.goal_position, self.current_lin_velocity)
                self.interpolate_orientation(self.current_orientation_mat, self.last_goal_orientation, self.goal_orientation, self.current_ang_velocity)
        ...
        if self.interpolate:
            ...
            self.last_goal_orientation = np.dot((T.euler2mat(-goal_orientation_delta).T), self.orientation_initial_interpolation)

            orientation_error = self.calculate_orientation_error(desired=self.last_goal_orientation, current=self.current_orientation_mat)
    ...
    def calculate_impedance_torques(self, position_error, orientation_error):
        ...
        if self.initial_joint is not None:
            # TODO where does 10 come from? what does initial joint mean?
            joint_kp = 10
    ...
    def update_model(self, sim, joint_index, id_name='right_hand'):
            ...
            self.update_mass_matrix(sim, joint_index)
            self.update_model_opspace(sim, joint_index)
            ...
            if np.array(self.orientation_limits).any():
                ...
                for idx in range(3):
                    if self.orientation_limits[0][idx] < self.orientation_limits[1][idx]:  #Normal angle sector meaning
                        if euler[idx] > self.orientation_limits[0][idx] and euler[idx] < self.orientation_limits[1][idx]:
                            continue
                        else:
                            limited = True
                            dist_to_lower = euler[idx] - self.orientation_limits[0][idx]
                            if dist_to_lower > np.pi: dist_to_lower -= 2*np.pi 
                            elif dist_to_lower < -np.pi: dist_to_lower += 2*np.pi

                            dist_to_higher = euler[idx] - self.orientation_limits[1][idx]
                            if dist_to_lower > np.pi: dist_to_higher -= 2*np.pi
                            elif dist_to_lower < -np.pi: dist_to_higher += 2*np.pi                             
