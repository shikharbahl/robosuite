...
class RobotArmEnv(MujocoEnv):
    ...
    def __init__(
        ...
        Args:
            ...
            initial_damping,
            initial_policy (TODO): given an observation, return an action from an existing agent
            robot_type (str): type of robot, used to instantiate
                robot models from robot factory.
            ...
            impedance_ctrl (bool): TODO
            controller (ControllerType): enum for which controller to use (see controller.py)
            use_impedance (bool): whether to control impedance as part of the action space. False
                for fixed impedance.
            use_delta_impedance (bool): True if using delta impedance as action as opposed
                to directly setting impedance.
        ...
        #self.initial_policy = initial_policy #TODO
        ...
        ########################## DATA LOGGING #################
            ...
            logger.info("filename for h5py: ", self.logging_filename)
            try:
            ...
            self.file_logging.create_dataset("transformed_action", (self.data_count, self.dof), maxshape=(None, self.dof))
            self.file_logging.create_dataset("joint_pos", (self.data_count, 7), maxshape=(None, len(self._ref_joint_vel_indexes)))
            self.file_logging.create_dataset("joint_vel", (self.data_count, 7), maxshape=(None, len(self._ref_joint_vel_indexes)))
            ...
            try:
                ...
                self.file_logging.create_dataset("impedance_kp", (self.data_count, impedance_dim), maxshape=(None, impedance_dim))
                self.file_logging.create_dataset("impedance_damping", (self.data_count, impedance_dim ), maxshape=(None, impedance_dim))             
                self.file_logging.create_dataset("impedance_kv", (self.data_count, impedance_dim ), maxshape=(None, impedance_dim))
            except:
    ...
    def _pre_action(self, action, policy_step):
        ...
        if self.initial_policy:
            if policy_step:
                # TODO - is this duplicating work?
                current_obs = self._get_observation()
                # account for prev-act being different dims in current policy env and initial policy env
                current_obs['prev-act'] = current_obs['prev-act'][:3]
                # need to remove extraneous observations from current env
                filtered_obs = np.expand_dims(self.initial_policy.env._flatten_obs(current_obs), axis=0)
                # get initial policy's action
                self.initial_policy_action = self.initial_policy.step(filtered_obs)[0][0]
            # use current model as a residual
            action[:3] += self.initial_policy_action # TODO - use action mask?
            # TODO - allow base environment to set impedance?
        '''
    ...
    def _post_action(self, action):
        ...
        avg_joint_torque = np.array(self.torque_total)/num_steps

        info['add_vals'].append('avg_joint_torque')
        info['avg_joint_torque'] = avg_joint_torque
        ...
        if self.data_logging:
            ...
            if self.counter < self.data_count:
                ...
                self.file_logging['joint_pos'][self.counter] = np.array([self.sim.data.qpos[x] for x in self._ref_joint_pos_indexes])
                self.file_logging['joint_vel'][self.counter] = np.array([self.sim.data.qvel[x] for x in self._ref_joint_vel_indexes])
                self.file_logging['ee_pos'][self.counter] = np.array(self.sim.data.body_xpos[self.sim.model.body_name2id('right_hand')])
                self.file_logging['ee_ori'][self.counter] = T.convert_quat(
                    self.sim.data.get_body_xquat("right_hand"), to="xyzw"
                )
                self.file_logging['ee_vel'][self.counter] = np.array(self.sim.data.body_xvelp[self.sim.model.body_name2id('right_hand')])
                self.file_logging['ee_vel_ori'][self.counter] = np.array(self.sim.data.body_xvelr[self.sim.model.body_name2id('right_hand')])
